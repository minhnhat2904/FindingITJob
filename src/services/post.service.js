...
export default class PostService {
  ...
  async deletePost(id) {
    ...
    await Post.findByIdAndUpdate(id, { deleted_flag: true });
    if (post.status == "ACCEPTED") {
      ...
      await Company.findByIdAndUpdate(post.companyId, {
        recruitingPost: numPost,
      });
    }
    return true;
  }
  ...
  async acceptPost(id) {
    ...
    const company = await Company.findByIdAndUpdate(accepted.companyId, {
      recruitingPost: numPost,
    });
    ...
    const notifications = followers.map((follower) => {
      return {
        ...
        title: `New job`,
        content: `${check.name} had posted a new job to looking for ${
          check.title
        } for ${check.skill.join(", ")}`,
        postId: check._id,
        userId: follower,
        image: company.image,
      };
    ...
    notifications.push({
      ...
      image:
        "https://res.cloudinary.com/do-an-cnpm/image/upload/v1629615468/ic_launcher_zxt0ie.png",
    });
    ...
    const itersHasFollowIncrement = itersHasFollow.map((iter) => {
      return ITer.findByIdAndUpdate(iter._id, {
  ...
  async applyPost(_id, iterId, cvId) {
    const existIter = await Post.findOne({
      _id,
  ...
  async listPostsByCompanyId(accountId) {
    return await Post.find(
      { accountId, status: "ACCEPTED", deleted_flag: false },
      {
        comment: 0,
        __v: 0,
        createdAt: 0,
  ...
  async listAppliedPosts(iterId) {
    ...
    posts = posts.map((post) => {
      post.status = post.apply.find(
        (iter) => JSON.stringify(iter.iterId) == JSON.stringify(iterId)
      ).status;
      post.apply = null;
      delete post.apply;
      return post;
  ...
  async savePost(iterId, postId) {
    ...
    if (save) {
      await SavedPost.findByIdAndUpdate(save._id, { deleted_flag: false });
      return false;
    }
